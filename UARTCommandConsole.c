...
void vUARTCommandConsoleStart( uint16_t usStackSize, UBaseType_t uxPriority )
{
	...
	//change with our xSemaphoreCreateMutex
...
//xComPortHandle xPort;
	...
	prvNewPrintString(( char * ) pcWelcomeMessage, ( unsigned short ) strlen( pcWelcomeMessage ));
	for( ;; )
	{
		/* Wait for the next character.  The while loop is used in case
		INCLUDE_vTaskSuspend is not set to 1 - in which case portMAX_DELAY will
		be a genuine block time rather than an infinite block time. */
		while( uart_messages_waiting() == 0 )
			{
				vTaskDelay(1);
				//prvNewPrintString("a",1);
			}
			
		cRxedChar = uart_getc();
		//prvNewPrintString(&cRxedChar,1);

		/* Ensure exclusive access to the UART Tx. */
		//if( xSemaphoreTake( uart_lock, cmdMAX_MUTEX_WAIT ) == pdPASS )
		//{
			...
			if( cRxedChar == '\n' || cRxedChar == '\r' )
			{
				...
				prvNewPrintString(( char * ) pcNewLine, ( unsigned short ) strlen( pcNewLine ));
				...
				do
				{
					...
					prvNewPrintString(( char * ) pcOutputString, ( unsigned short ) strlen( pcOutputString ));
				...
				prvNewPrintString(( char * ) pcEndOfOutputMessage, ( unsigned short ) strlen( pcEndOfOutputMessage ));
			}
			else
			{
				if( cRxedChar == '\r' )
